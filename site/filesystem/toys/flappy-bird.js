(async () => {
  ...
  const flapDown = 5;
  const flapCycle = [
      ...Array(flapUp).fill().map((_) => "BB Upflap"),
      ...Array(flapDown).fill().map((_) => "BB Midflap"),
      ...Array(flapMid).fill().map((_) => "BB Downflap"),
  ];
  ...
  const draw = (sprite, dx, dy, rot) => {
    ...
    ctx.drawImage(sprites, x, y, width, height, -width / 2, -height / 2, width, height);
  ...
  const tick = (td) => {
    ...
    let ua, da = td * gravityAcceleration;

    if (flapping) ua = td * flapAcceleration;
    else ua = 0;

    acceleration = da + ua
    velocity = clamp(velocity + acceleration, -terminalVelocity, terminalVelocity);
  ...
  const checkCollisions = () => {
    ...
    for (let p of [p1, p2]) {
      ...
      const bottomRect = { x: p[0], y: p[1]-320-verticalPipeGap, w: 52, h: 320 };
      if (
