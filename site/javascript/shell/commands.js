...
import { makeAbsolute, parent, join } from "./utils.js";
import { env } from "./env.js";

export function makeCommands(shell) {
  const helpMessage = `\
  ...
  const ls = (argc, argv) => {
    const currentDirectory = shell.pwdValue;
    let stdout = "";
    ...
    for (let target of targets) {
      let path = makeAbsolute(shell.pwdValue, target);
  ...
  const cat = (argc, argv) => {
    ...
    for (let file of argv) {
      let target = makeAbsolute(shell.pwdValue, file);
  ...
  const open = (argc, argv) => {
    ...
    let target = makeAbsolute(shell.pwdValue, argv[0]);
    if (Object.keys(fileSystem.files).includes(target)) {
  ...
  const _tree = (root, prefix) => {
    ...
    for (let [i, node] of nodes.entries()) {
      ...
      if (node.endsWith("/")) {
        ...
        let prefixPart = (i === nodes.length - 1) ?  "    " :   "│   ";
        contents = contents.concat(_tree(dir, prefix + prefixPart));
      }
    }
    return contents;
  };
  ...
  const tree = (argc, argv) => {
    ...
    if (argc === 0) {
      ...
      root = makeAbsolute(shell.pwdValue, argv[0]);
      vroot = argv[0];
    }
    if (fileSystem.dirs[root] === undefined) return response("",  `${vroot} [error opening dir]`, 0);;
